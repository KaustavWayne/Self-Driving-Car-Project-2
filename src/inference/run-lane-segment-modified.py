...
class SteeringWheelDisplay:
    ...
    def show(self, angle):
        h, w = self.steering_image.shape[:2]
...
# MAIN PIPELINE
# ==========================================================
def run_pipeline(dataset_folder, yolo_model_path, steering_model_path, steering_wheel_path):
    ...
    image_files = [img for img in os.listdir(dataset_folder) if img.endswith(".jpg") or img.endswith(".png")]
    image_files.sort()

    cv2.namedWindow("Lane + Segmentation + Steering", cv2.WINDOW_NORMAL)
    cv2.resizeWindow("Lane + Segmentation + Steering", 700, 450)
    ...
    for image_file in image_files:
        ...
        predicted_angle = steering_predictor.predict_angle(image)
        ...
        seg_output = draw_segmentation_mask(result, lane_output)
        ...
        cv2.imshow("Lane + Segmentation + Steering", seg_output)
        steering_display.show(smoothed_angle)

        print(f"{image_file} | Steering: {smoothed_angle:.2f} deg")
...
if __name__ == "__main__":
    ...
    dataset_folder = os.path.join(ROOT, "data", "driving_dataset", "IMG")

    yolo_model_path = os.path.join(ROOT, "saved_models", "lane_segmentation", "lane_seg_best.pt")
    ...
    run_pipeline(dataset_folder, yolo_model_path, steering_model_path, steering_wheel_path)
